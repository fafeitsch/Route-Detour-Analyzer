 ...
 */
import { NotificationService, Timetable } from '../../shared';
...
@Injectable()
export class TimetableListStore extends ComponentStore<State> {
  readonly timetables$ = super.select((state) => state.timetables);
  readonly lineKey$ = super.select((state) => state.lineKey);
  ...
  readonly createTimetable$ = super.effect((trigger$: Observable<void>) =>
    trigger$.pipe(
      withLatestFrom(this.lineKey$.pipe(isDefined())),
      switchMap(([, lineKey]) =>
        this.service.saveTimetable({ lineKey }).pipe(
          tapResponse(
            (timetable) => {
              this.notificationService.raiseNotification(
                'Timetable created',
  ...
  readonly getLineKeyFromRoute = super.effect(() =>
    this.route.paramMap.pipe(
      map((params) => params.get('line')),
      tap((lineKey) => super.patchState({ lineKey: lineKey || undefined }))
    )
  );
  ...
  constructor(
    ...
    super({ timetables: [], lineKey: undefined });
  }
}
