...
import { Station, Waypoint } from './shared';
...
export class DetourService {
  ...
  computeDetours(path: Waypoint[], paths: SubPath[]): DetourResult {
    if (!paths.length) {
      return { averageDetour: 0 };
    }
    const originalDistances = this.computeDistanceTable(path);
    const mapToLength = (sub: SubPath) =>
      this.computeDistanceTable(sub.path)[0][1];
    ...
    const averageDetour =
      detailedResults
        ...
        .reduce((acc, curr) => acc + curr, 0) / detailedResults.length;
    return {
      averageDetour,
  ...
  createQueryPairs(line: Station[], cap: number): QueryPair[] {
    ...
    line.forEach((stop, index) => {
      ...
      line.slice(index + 1).forEach((target, targetIndex) => {
        ...
        result.push({
          source: { ...stop, index },
          target: { ...target, index: targetIndex + index + 1 },
        });
      });
    });
    return result;
  ...
  private computeDistanceTable(path: Waypoint[]) {
    const legs = this.routeService.buildRouteLegs(path);
    const result: number[][] = [
      ...legs.map((_) => [...legs.map((_) => 0), 0]),
      [0],
    ];
